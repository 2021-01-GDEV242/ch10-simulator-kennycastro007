...
public class Simulator {
    ...
    // The probability that a honey badger will be created in any given grid
    // position.
    private static final double HONEY_BADGER_CREATION_PROBABILITY = 0.12;
    ...
    public Simulator(int depth, int width) {
        ...
        view.setColor(HoneyBadger.class, Color.PINK);
        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                if (rand.nextDouble() <= FOX_CREATION_PROBABILITY) {
                    ...
                    animals.add(rabbit);
                } else if (rand.nextDouble() <= HONEY_BADGER_CREATION_PROBABILITY) {
                    Location location = new Location(row, col);
                    HoneyBadger beast = new HoneyBadger(field, location);
                    animals.add(beast);
                }
